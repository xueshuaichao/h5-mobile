<template>
    <div class="filterpage">
        <div class="filterpage-header">
            <h3>课程筛选</h3>
            <img @click="goList" src="../../assets/course/seach@2x.png" alt="">
        </div>
        <van-dropdown-menu active-color="#E85A3A" :overlay='false'>
            <van-dropdown-item :title="seachparam.categorytitle">
                <div class="classify clearfix">
                    <div class="classify-left fl">
                        <ul>
                            <li :class="seachparam.firstClass === null ? 'activeCalss' : ''" @click="getChildren()">
                                <span class="line"></span>
                                全部
                                <span class="top-yuan yuan"></span>
                                <span class="bottom-yuan yuan"></span>
                            </li>
                            <li :class="seachparam.firstClass === item.id ? 'activeCalss' : ''" @click="getChildren(item)" v-for="(item, index) in classifyparam" :key="index">
                                <span class="line"></span>
                                {{item.name}}
                                <span class="top-yuan yuan"></span>
                                <span class="bottom-yuan yuan"></span>
                    ...
                    <div class="classify-right fl">
                        <span :class="seachparam.secondClass === null ? 'activeCalss' : ''" @click="getSecond()">全部</span>
                        <span :class="seachparam.secondClass === child.id ? 'activeCalss' : ''"  @click="getSecond(child)" v-for="child in secondCategory"
                            :key="child.id">
                            {{child.name}}
                        </span>
                    </div>
                ...
                <div class="filters">
                    <div class="filters-type">
                        <div class="filters-status">
                            <p>选择状态</p>
                            <span :class="statusId === item.id ? 'activespan' : ''" @click="getStatus(item)" v-for="(item, index) in statuslist" :key="index">
                                {{item.name}}
                            </span>
                        </div>
                        <div class="filters-status">
                            <p>内容类型</p>
                            <span :class="typeId === item.type ? 'activespan' : ''" @click="getType(item)" v-for="(item, index) in contentType" :key="index">
                                {{item.name}}
                            </span>
                        </div>
                    </div>
                    <div class="footer clearfix">
                        <div class="footer-left fl" @click="handleClearActiveList">
                            <img src="../../assets/learning/clear.png" alt="" width="18" srcset="">
                            <p>清空筛选</p>
                        </div>
                        <div class="button fr" @click="handleClickSave">
                            确定
                        </div>
                    </div>
                </div>
            ...
            <van-dropdown-item v-model="sortparam" title="综合排序">
                <div class="sortStatus">
                    <span :class="sortId === item.id ? 'activesort' : ''" @click="getsort(item)" v-for="(item, index) in sortType" :key="index">
                        {{item.name}}
                    </span>
                </div>
            </van-dropdown-item>
